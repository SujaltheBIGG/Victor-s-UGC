    ...
    <div className={wrapperClass}>
      <div className="w-full max-w-sm bg-[#0a0a0a]/90 backdrop-blur-xl border border-white/10 rounded-xl p-10 shadow-2xl relative">
        ...
        <div className="flex flex-col items-center text-center mb-10">
          <div className="w-14 h-14 bg-[#b0fb5d]/5 rounded-2xl flex items-center justify-center border border-[#b0fb5d]/10 mb-6 group hover:border-[#b0fb5d]/30 transition-colors">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#b0fb5d" strokeWidth="1.5" className="group-hover:scale-110 transition-transform">
          ...
          <p className="text-white/40 text-[13px] leading-relaxed px-4">
            {subtitle || (
              <>{copy.subtitlePrefix} <a href="https://muapi.ai/access-keys" target="_blank" rel="noreferrer" className="text-[#b0fb5d] hover:text-[#e5ff33] transition-colors">Muapi.ai</a> {copy.subtitleSuffix}</>
            )}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/[0.03] rounded-md px-5 py-3 text-sm text-white placeholder:text-white/10 focus:outline-none focus:ring-1 focus:ring-[#b0fb5d]/30 focus:bg-white/[0.07] transition-all"
          ...
          <button
            type="submit"
            className="w-full bg-[#b0fb5d] text-black font-medium py-2.5 rounded-md hover:bg-[#e5ff33] hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-[#b0fb5d]/5"
            suppressHydrationWarning
          >
            {copy.submit}
          </button>
          ...
          <p className="text-center text-[12px] text-white/20 pt-2">
            {copy.needKey}{' '}
            <a href="https://muapi.ai/access-keys" target="_blank" rel="noreferrer" className="text-white/40 hover:text-[#b0fb5d] transition-colors font-medium">
              {copy.getOneFree}
            </a>
          </p>
        </form>
