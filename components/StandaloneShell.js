...
export default function StandaloneShell({ locale = 'en' }) {
  ...
  const fetchBalance = useCallback(async (key) => {
    try {
      ...
      // An unusable key just leaves the balance unknown: the header shows its
      // placeholder and the 30s poll picks up a working key later.
      if (err?.authRequired) console.warn('Balance unavailable:', err.message);
      else console.error('Balance fetch failed:', err);
    }
  }, []);
  ...
  return (
    ...
    >
      {/* Drag Overlay */}
      {isDragging && (
        <div className="fixed inset-0 z-[100] bg-[#b0fb5d]/10 backdrop-blur-md border-4 border-dashed border-[#b0fb5d]/50 flex items-center justify-center pointer-events-none transition-all duration-300">
          <div className="bg-[#0a0a0a] p-8 rounded-3xl border border-white/10 shadow-2xl flex flex-col items-center gap-4 scale-110 animate-pulse">
            <div className="w-20 h-20 bg-[#b0fb5d] rounded-2xl flex items-center justify-center">
      ...
      {/* Vadoo promo banner */}
      {showVadooBanner && (
        <div className="flex-shrink-0 w-full bg-lime-600 flex items-center justify-center px-4 py-2 gap-3 relative z-50">
          <a
            href="https://vadoo.tv"
            target="_blank"
        ...
        <header className="flex-shrink-0 h-14 border-b border-white/[0.05] flex items-center justify-between px-4 bg-[#0a0a0b]/80 backdrop-blur-md z-50 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 bg-[#b0fb5d] rounded-lg flex items-center justify-center shadow-lg shadow-[#b0fb5d]/20">
          ...
          <div className="hidden lg:flex items-center gap-2 px-3 py-1 rounded-full bg-white/[0.03] border border-white/[0.05] text-xs text-white/60">
            <span className="w-1.5 h-1.5 rounded-full bg-[#b0fb5d]" />
                    ...
                    <div key={category.id} className="relative">
                      <button
                        ...
                        className={`
                          ...
                          ${isCategoryActive
                            ? 'bg-gradient-to-r from-[#b0fb5d]/15 to-purple-500/10 text-[#b0fb5d] border border-[#b0fb5d]/20 shadow-[0_0_15px_rgba(176,251,93,0.08)]'
                            : isCategoryOpen
                      ...
                      >
                        {isCategoryActive && (
                          <span className="absolute left-0 top-2 bottom-2 w-1 bg-gradient-to-b from-[#b0fb5d] to-[#a855f7] rounded-r-full shadow-[0_0_8px_rgba(176,251,93,0.6)]" />
                        )}

                        <span className={`flex-shrink-0 transition-colors ${isCategoryActive ? 'text-[#b0fb5d]' : 'text-white/55 group-hover:text-white'}`}>
                          {category.icon}
                        </span>
                            ...
                            return (
                              <a
                                ...
                                className={`
                                  ...
                                  ${isActive
                                    ? 'bg-[#b0fb5d]/12 text-[#b0fb5d] border border-[#b0fb5d]/20'
                                    : 'text-white/55 hover:text-white hover:bg-white/[0.04] border border-transparent'
                                  }
                                `}
                              >
                                {isActive && (
                                  <span className="absolute -left-[11px] top-2 bottom-2 w-0.5 rounded-full bg-[#b0fb5d] shadow-[0_0_7px_rgba(176,251,93,0.7)]" />
                                )}
                                <span className={`flex-shrink-0 ${isActive ? 'text-[#b0fb5d]' : 'text-white/45 group-hover:text-white/80'}`}>
                                  {tab.icon}
                                </span>
                  ...
                  <a
                    ...
                    className={`
                      ...
                      ${activeTab === EXPLORE_APPS_TAB.id
                        ? 'bg-gradient-to-r from-[#b0fb5d]/15 to-purple-500/10 text-[#b0fb5d] border border-[#b0fb5d]/20'
                        : 'text-white/60 hover:text-white hover:bg-white/[0.04] border border-transparent'
                      }
                    `}
                  >
                    {activeTab === EXPLORE_APPS_TAB.id && (
                      <span className="absolute left-0 top-2 bottom-2 w-1 bg-gradient-to-b from-[#b0fb5d] to-[#a855f7] rounded-r-full" />
                    )}
                    <span className={`flex-shrink-0 ${activeTab === EXPLORE_APPS_TAB.id ? 'text-[#b0fb5d]' : 'text-white/50 group-hover:text-white'}`}>
        ...
        >
          {activeGenerations.map((generation) => (
            <div
              ...
              className="pointer-events-auto flex items-center gap-3 rounded-xl border border-lime-500/40 bg-white px-3.5 py-3 text-[13px] text-zinc-900 shadow-[0_10px_30px_rgba(0,0,0,0.15)]"
              data-testid="generation-activity"
            >
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-lime-400/40 bg-lime-50">
                <span
                  className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-lime-600/30 border-t-cyan-600"
                  aria-hidden="true"
                />
              </span>
          ...
          {notifications.map((notif) => (
            <div
              ...
              style={{
                borderColor: notif.type === 'success' ? 'rgba(143,217,63,0.4)' : 'rgba(239,68,68,0.4)',
            ...
            >
              <span
                className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border ${
                  notif.type === 'success'
                    ? 'border-lime-400/40 bg-lime-50 text-lime-600'
              ...
              <div className="min-w-0 flex-1">
                ...
                {notif.type === 'success' && (
                  <button
                    ...
                    className="mt-1.5 text-[11px] font-bold text-lime-600 transition-colors hover:text-lime-700"
