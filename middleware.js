...
// The marketing landing page at / is a static document that pulls its CSS,
// fonts, and media from an external CDN, so it needs a wider policy than the
// studio app. Everything else keeps the strict 'self' policy above.
function addMarketingHeaders(response) {
    response.headers.set('X-Content-Type-Options', 'nosniff');
    response.headers.set('Referrer-Policy', 'strict-origin-when-cross-origin');
    response.headers.set(
        'Content-Security-Policy',
        "default-src 'self'; " +
        "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.magnific.com; " +
        "style-src 'self' 'unsafe-inline' https://cdn.magnific.com https://fonts.googleapis.com; " +
        "img-src 'self' data: blob: https:; " +
        "media-src 'self' data: blob: https:; " +
        "font-src 'self' data: https://cdn.magnific.com https://fonts.gstatic.com; " +
        "connect-src 'self' https://*.magnific.com; " +
...
export function middleware(request) {
    ...
    // Serve the landing page at the site root; the studio lives at /studio.
    if (url.pathname === '/') {
        const rewrite = NextResponse.rewrite(new URL('/marketing/index.html', url));
        return addMarketingHeaders(rewrite);
    }
