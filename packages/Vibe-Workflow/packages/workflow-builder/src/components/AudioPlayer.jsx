    ...
    <div className={className || "flex flex-col items-center justify-center p-4 w-full h-full bg-gradient-to-br from-[#121418] to-[#08090a] rounded-xl border border-white/5 relative group transition-all duration-500 select-none"}>
        ...
        {bars.map((height, i) => (
          <div 
            ...
            style={{ 
              ...
              backgroundColor: (i / bars.length) < (progress / 100) ? '#a3e635' : '#2c3037',
              boxShadow: (i / bars.length) < (progress / 100) ? '0 0 10px rgba(163, 230, 53, 0.4)' : 'none',
      ...
      <div className="flex items-center gap-4 w-full relative z-10">
        <button
          ...
          onClick={toggleAudio}
          className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-lime-600 hover:bg-lime-500 text-black rounded-full transition-all duration-300 shadow-[0_0_20px_rgba(132,204,22,0.3)] hover:scale-110 active:scale-95 group/play"
        >
          {isPlaying ? (
        ...
        <div className="flex flex-col flex-grow gap-1.5 min-w-0">
          <input
            ...
            style={{
              background: `linear-gradient(to right, #a3e635 0%, #a3e635 ${progress || 0}%, rgba(255, 255, 255, 0.1) ${progress || 0}%, rgba(255, 255, 255, 0.1) 100%)`
            }}
          />
          ...
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 pb-4 opacity-0 group-hover/volume:opacity-100 pointer-events-none group-hover/volume:pointer-events-auto transition-all duration-300 translate-y-2 group-hover/volume:translate-y-0 z-30">
            <div className="bg-[#1a1b1e] border border-white/10 p-3 rounded-lg shadow-2xl backdrop-blur-xl flex flex-col items-center gap-2 h-24">
              <input 
                ...
                className="h-full w-1 accent-lime-500 cursor-pointer appearance-none bg-white/10 rounded-full"
                style={{
