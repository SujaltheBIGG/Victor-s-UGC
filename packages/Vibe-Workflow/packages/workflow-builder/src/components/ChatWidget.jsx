    ...
    <div ref={widgetRef} className="fixed bottom-10 right-10 z-50 flex flex-col items-end gap-2 font-sans">
      {isOpen && (
        <div className={`${isWide ? 'w-[800px]' : 'w-[380px]'} max-w-[100vw] h-[600px] max-h-[100%] flex flex-col bg-[#0B0F17]/95 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden transition-all duration-300 animate-in slide-in-from-bottom-5 fade-in text-left`}>
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-purple-600/20 to-lime-600/20 border-b border-white/10">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-lime-600 rounded-lg shadow-lg">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="hidden md:flex p-2 text-gray-400 hover:text-lime-400 transition-colors rounded-full hover:bg-white/5"
              >
              ...
              <div className="flex flex-col items-center justify-center gap-6 text-center p-6 h-full">
                <div className="flex flex-col items-center gap-2 text-gray-500">
                  <div className="p-2 bg-white/5 rounded-2xl border border-white/5 shadow-inner">
                    <FaRobot className="text-3xl text-lime-400 opacity-80" />
                  </div>
                ...
                <div className={`grid ${isWide ? "grid-cols-2" : "grid-cols-1"} gap-2 w-full`}>
                  {DEFAULT_SUGGESTIONS.map((suggestion, sIdx) => (
                    <button
                      ...
                      className="px-4 py-3 text-xs font-medium bg-white/5 text-gray-300 rounded-xl hover:bg-lime-600/20 hover:text-lime-400 hover:border-lime-500/50 transition-all text-left border border-white/10 shadow-sm cursor-pointer flex items-center gap-3 group"
                    >
                      <BsStars size={12} className="text-lime-400/50 group-hover:text-lime-400 transition-colors" />
                      {suggestion}
                    </button>
                  ))}
                </div>
                    ...
                    >
                      <div
                        className={`flex flex-col gap-4 max-w-[85%] p-3 rounded-2xl text-sm leading-relaxed shadow-sm break-words whitespace-pre-wrap ${
                          msg.role === "user"
                            ? "bg-lime-600 text-black rounded-br-none"
                      ...
                      >
                        <ReactMarkdown 
                          ...
                          components={{
                            ...
                            h3: ({node, ...props}) => <h3 className="text-base font-bold text-lime-400" {...props} />,
                            ...
                            a: ({node, ...props}) => <a className="text-lime-400 hover:text-lime-300 underline underline-offset-4 decoration-lime-500/30 break-all transition-colors" target="_blank" rel="noopener noreferrer" {...props} />,
                            ...
                            th: ({node, ...props}) => (
                              <th className="px-4 py-3 bg-gradient-to-b from-white/10 to-white/5 text-left text-[11px] font-bold uppercase tracking-wider text-lime-400 border-b border-white/10" {...props} />
                            ),
                            td: ({node, ...props}) => (
                            ...
                            <div className="flex flex-wrap gap-2 pt-2 border-t border-white/10">
                              {msg.suggestions.map((suggestion, sIdx) => (
                                <button
                                  ...
                                  key={sIdx}
                                  onClick={() => onSendMessage(suggestion)}
                                  className="px-3 py-1.5 text-xs font-medium bg-[#242936] text-gray-300 rounded-lg hover:bg-lime-600/20 hover:text-lime-400 transition-colors text-left border border-white/10 shadow-sm cursor-pointer"
                                >
                                  {suggestion}
                                </button>
                              ))}
                      ...
                      <div className="flex items-center gap-2 mt-1 px-1">
                        ...
                        <button
                          ...
                          className="text-gray-400 hover:text-lime-500 transition-colors cursor-pointer"
                          title="Copy Message"
                        >
                ...
                <div className="flex flex-col gap-1">
                  <div className="bg-[#1A1F2B]/80 backdrop-blur-sm p-3.5 rounded-2xl rounded-bl-none border border-white/5 shadow-sm flex items-center gap-2 min-w-[70px]">
                    <div className="flex gap-1.5">
                      <span className="w-1.5 h-1.5 bg-lime-500 rounded-full animate-bounce [animation-delay:-0.3s]" />
          ...
          <div className="p-4 bg-white/5 border-t border-white/10">
            <form
              ...
              className="flex items-center gap-2 bg-[#0B0F17] border border-white/10 rounded-xl px-2 py-2 focus-within:ring-2 focus-within:ring-lime-500/50 transition-all"
            >
              <textarea
                ref={inputRef}
                value={inputValue}
              ...
              <button
                ...
                className="p-2 bg-lime-600 text-black rounded-full shadow-md hover:bg-lime-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all transform active:scale-95 shrink-0"
              >
      ...
      {!isOpen && (
        <button
          ...
          suppressHydrationWarning={true}
          onClick={toggleChat}
          className={`group relative right-6 md:right-0 flex items-center justify-center w-10 h-10 bg-lime-600 rounded-full shadow-lg shadow-lime-600/30 hover:shadow-lime-600/50 hover:scale-110 transition-all duration-300 ${isLoading ? 'ring-2 ring-lime-200 ring-offset-2' : ''}`}
        >
          {isLoading ? (
