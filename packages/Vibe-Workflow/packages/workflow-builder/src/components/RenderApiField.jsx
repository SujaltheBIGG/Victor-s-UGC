...
const RenderApiField = ({ fieldName, meta, idx, formValues, setFormValues, handleChange, hasHandle = false, exposedHandles = [], onToggleHandle }) => {
    ...
    <div className="flex items-center justify-between w-full group/label">
      ...
      {onToggleHandle && (
        <button
          ...
          className={`p-1 rounded-lg transition-all group-hover/label:opacity-100 h-6 w-6 flex items-center justify-center ${exposedHandles.includes(fieldName) ? "text-lime-500 bg-lime-500/10 opacity-100" : "text-zinc-500 hover:text-white hover:bg-white/5 opacity-0"}`}
  ...
  //           className="h-1 rounded-full cursor-pointer accent-lime-600 active:accent-lime-600 outline-none w-full"
  //         />
  //         <input 
  //           type="number" 
  //           id={fieldName} 
    ...
    return (
      <div key={fieldName} className="flex flex-col gap-1 relative">
        ...
        <input
          ...
          className="bg-zinc-900/50 text-white text-xs p-2 rounded-lg border border-white/10 hover:border-white/20 transition-all outline-none focus:border-lime-500/50"
        />
      </div>
    );
  };
