...
const RenderField = ({ fieldName, meta, idx, formValues, setFormValues, handleChange, data, modelName }) => {
  ...
  const label = (
    <label className="text-[10px] font-bold text-zinc-500 text-start px-1 mb-1">
      ...
      {isRequired && <span className="text-lime-500 text-[9px] ml-1">* required</span>}
    </label>
  );
          ...
          >
            {meta.enum.map((option, i) => (
              ...
              >
                ...
                {formValues[fieldName] === option && (
                  <span className="ml-auto text-lime-400 font-bold">✓</span>
                )}
              </button>
            ))}
          </div>
      ...
      <div key={fieldName} className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-1">
          <input 
            ...
            className="bg-zinc-900/50 text-white text-xs py-2 px-3 rounded-lg border border-white/10 transition-all hover:border-white/20 w-full outline-none focus:border-lime-500/50" 
          ...
          <label 
            ...
            className={`flex items-center justify-center gap-1 bg-lime-500 text-white hover:bg-lime-600 text-xs font-medium cursor-pointer flex-shrink-0 ${
        ...
        {uploading && (
          <div className="w-full bg-gray-700/70 rounded h-1 overflow-hidden">
            <div className="bg-lime-500 h-full" style={{ width: `${uploadProgress}%` }}></div>
          </div>
        )}
      ...
      <div key={fieldName} className="flex flex-col w-full">
        ...
        <div className="flex items-center gap-2 w-full">
          <input
            ...
            className="h-1.5 rounded-full cursor-pointer accent-lime-600 outline-none w-full bg-zinc-800"
          />
          <input 
            type="number" 
            id={fieldName} 
            ...
            // }} 
            className="w-12 h-8 text-center text-white rounded-lg border border-white/10 text-[10px] font-bold bg-zinc-900/50 outline-none focus:border-lime-500/50 transition-all" 
          />
        </div>
      </div>
    );
    ...
    return (
      <div key={fieldName} className="flex flex-col gap-1 w-full">
        <label htmlFor={fieldName} className="flex items-center gap-2 text-sm text-white font-medium relative">
          {label}
          {isRequired && <span className="text-lime-500 text-xs">* required</span>}
        </label>
        <div className="flex items-center gap-2 w-full">
          <input 
            type="number" 
            ...
            className="w-full rounded-lg border border-white/10 px-3 py-2 text-white text-xs bg-zinc-900/50 hover:border-white/20 focus:border-lime-500/50 outline-none transition-all" 
          />
        </div>
      </div>
    );
    ...
    return (
      <div key={fieldName} className="flex flex-col gap-2 w-full">
        ...
        <input
          ...
          onChange={(e) => handleChange(fieldName, e.target.value)}
          className="bg-zinc-900/50 text-white text-xs py-2 px-3 rounded-lg border border-white/10 hover:border-white/20 transition-all w-full outline-none focus:border-lime-500/50"
        />
      </div>
    );
  };
      ...
      <div key={fieldName} className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor={`instrumental-${fieldName}`} className="flex items-center justify-between cursor-pointer select-none relative shrink-0">
            ...
            />
            <span className={`flex items-center h-[20px] w-[36px] rounded-full p-1 duration-200 transition-all ${!!formValues[fieldName] ? "bg-lime-600 shadow-lg shadow-lime-900/40" : "bg-zinc-800 border border-white/10"}`}>
