...
const TextGeneration = ({ id, data, selected }) => {
  ...
  return (
    <div 
      style={{ minHeight: 280, '--loader-color': '#84cc16' }} 
      className={`
        nowheel group flex flex-col flex-1 w-80 
        rounded-2xl border-2 relative transition-all duration-300 ease-in-out 
        ${selected 
          ? "border-lime-600 shadow-[0_0_25px_rgba(132,204,22,0.3)] scale-[1.02] ring-1 ring-lime-500/20" 
    ...
    >
      ...
      <div className="flex items-center gap-2 absolute -top-5 left-0">
        ...
        {generationCost !== null && !selectedModel?.id.includes("passthrough") && (
          <span className="text-xs text-lime-500 -mt-0.5 font-medium flex items-center gap-1 opacity-80">
            {isRefreshingCost ? (
              <span className="flex items-center gap-1 italic text-lime-200">
                <div className="w-2 h-2 border-[1.5px] border-lime-200/30 border-t-blue-400 rounded-full animate-spin"></div>
              </span>
            ) : (
              <span>
      ...
      <div className="flex flex-col">
        <div className="flex items-center justify-between bg-gradient-to-r from-[#151618] to-[#1c1e21] rounded-t-2xl border-b border-zinc-800 py-2 px-3">
          <div className="flex items-center gap-2.5">
            <div className={`p-1.5 rounded-lg ${selected ? "bg-lime-600 text-black" : "bg-zinc-800 text-zinc-400"} transition-colors`}>
              <TfiText size={14} />
            </div>
              ...
              {currentOutputList.length > 1 && (
                <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/60 backdrop-blur-sm px-2 py-0.5 rounded-full border border-white/10 opacity-0 group-hover/text:opacity-100 transition-opacity">
                  <button
                    ...
                    className="text-white hover:text-lime-400 p-0.5"
                  >
                    <FaAngleLeft size={12} />
                  </button>
      ...
      <Handle  
        ...
        className={`
          ...
          ${connectedInputs.textInput 
            ? '!bg-lime-500 !border-zinc-900 shadow-[0_0_15px_rgba(163,230,53,0.8)]' 
            : '!bg-zinc-900 !border-lime-500/50 hover:!border-lime-500 shadow-sm'
          }
        `}
        data-type="blue"
      />
      {hasPrompt && (
        <p 
          className={`absolute -left-9 top-[100px] text-[10px] font-bold tracking-tight text-lime-500 transition-all duration-300 ${
      ...
      <Handle  
        ...
        className={`
          ...
          ${connectedInputs.textInput4 
            ? '!bg-lime-600 !border-zinc-900 shadow-[0_0_15px_rgba(132,204,22,0.8)]' 
            : '!bg-zinc-900 !border-lime-600/50 hover:!border-lime-600 shadow-sm'
          }
        `}
        data-type="blue"
      />
      {hasSystemPrompt && (
        <p 
          className={`absolute -left-14 top-[250px] text-[10px] font-bold tracking-tight text-lime-600 transition-all duration-300 ${
            data.activeHandleColor === "blue" 
              ? "opacity-100 translate-x-0" 
              : "opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0"
          }`}
      ...
      />
      <p 
        className={`absolute -right-9 top-[100px] text-[10px] font-bold tracking-tight text-lime-500 transition-all duration-300 ${
          data.activeHandleColor === "blue" 
            ? "opacity-100 translate-x-0" 
            : "opacity-0 translate-x-1 group-hover:opacity-100 group-hover:translate-x-0"
        }`}
