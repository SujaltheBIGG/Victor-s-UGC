...
export default function AppsStudio({ apiKey, locale = 'en' }) {
  ...
  useEffect(() => {
    if (apiKey) {
      getAppInterests(apiKey)
        ...
        .catch(err => {
          // Without a usable key there is nothing to restore — the list just
          // starts empty rather than surfacing a fault the user can't act on.
          if (err?.authRequired) console.warn("App interests unavailable:", err.message);
          else console.error("Error fetching interests:", err);
        });
    }
  }, [apiKey]);
  ...
  const renderAppCard = (app, isDummy = false, index = 0) => {
    ...
    const gradients = [
      "from-lime-600/20 to-lime-600/20",
    ...
    return (
      <div 
        key={app.name}
        className="group bg-[#0a0a0a] border border-white/5 rounded-lg flex flex-col overflow-hidden transition-all duration-300 hover:border-white/10 hover:bg-[#0f0f0f] hover:shadow-2xl hover:shadow-lime-500/5 hover:-translate-y-1"
      >
        {/* Thumbnail Section */}
        ...
        <div className="p-5 flex flex-col flex-1 space-y-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-lg text-[#b0fb5d] border border-white/5 group-hover:border-white/10 transition-colors">
              <app.icon />
            </div>
          ...
          <div className="flex items-center gap-2 pt-2">
            {isDummy ? (
              <>
                ...
                <button
                  onClick={() => setSelectedApp(app)}
                  className="flex-1 py-2 bg-[#b0fb5d]/10 text-[#b0fb5d] rounded-md text-[11px] font-bold uppercase tracking-wider flex items-center justify-center gap-2 hover:bg-[#b0fb5d]/20 transition-all border border-[#b0fb5d]/20 active:scale-95"
                >
    ...
    <div className="h-full w-full flex flex-col items-center bg-[#030303] overflow-y-auto custom-scrollbar relative">
      ...
      <div className="flex flex-col gap-10 items-center w-full max-w-7xl pt-12 pb-24 px-6">
        ...
        <div className="text-center space-y-6 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#b0fb5d]/10 border border-[#b0fb5d]/20 rounded-full">
            <FaDollarSign className="text-[#b0fb5d] text-xs" />
            <span className="text-[10px] font-black text-[#b0fb5d] uppercase tracking-widest">{copy.hero.badge}</span>
          </div>
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={step} className="flex items-start gap-4 bg-[#0a0a0a] border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-colors">
              <div className="w-12 h-12 shrink-0 rounded-2xl bg-white/5 flex items-center justify-center text-[#b0fb5d] border border-white/5">
        ...
        <div className="pt-24 pb-12 flex flex-col items-center gap-4">
          <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 rounded-full border border-white/5">
            <span className="block w-1.5 h-1.5 rounded-full bg-[#b0fb5d] animate-pulse" />
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
          ...
          <div className="relative bg-[#0a0a0a] border border-white/10 w-full max-w-md rounded-2xl p-8 space-y-8 animate-scale-up shadow-2xl">
            <div className="flex flex-col items-center text-center space-y-4">
              <div className="w-20 h-20 rounded-[28px] bg-[#b0fb5d]/10 border border-[#b0fb5d]/20 flex items-center justify-center text-4xl text-[#b0fb5d] mb-2">
                <selectedApp.icon />
              </div>
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-4 bg-[#b0fb5d] text-black rounded-md text-sm font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-[#b0fb5d]/90 transition-all shadow-lg active:scale-95 disabled:opacity-50"
              >
