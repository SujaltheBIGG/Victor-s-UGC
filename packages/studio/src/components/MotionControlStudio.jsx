...
// ── Icons ─────────────────────────────────────────────────────────────────────
const VideoIcon = ({ className = "text-white/40 group-hover:text-[#b0fb5d] transition-colors" }) => (
...
const ImageIcon = ({ className = "text-white/40 group-hover:text-[#b0fb5d] transition-colors" }) => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
const SlidersIcon = ({ className = "text-white/50 group-hover:text-[#b0fb5d] transition-colors" }) => (
    ...
    <button
      ...
      className={promptMediaButtonClassName({
        ...
        className: isDragging
          ? "border-[#b0fb5d] bg-[#b0fb5d]/20 ring-2 ring-[#b0fb5d]/50 scale-105"
          : uploadState === UPLOAD_STATE.READY
          ? "border-[#b0fb5d] ring-1 ring-[#b0fb5d]/40"
          : "",
      })}
    >
      <input
        ...
        <div className="flex flex-col items-center justify-center w-full h-full absolute inset-0 bg-black/80 z-20 backdrop-blur-[2px]">
          <svg className="w-8 h-8 -rotate-90">
            ...
            <circle
              ...
              className="text-[#b0fb5d] transition-all duration-300"
            />
          </svg>
          <span className="absolute text-[9px] font-black text-[#b0fb5d] leading-none">
            {progress}%
          </span>
        </div>
      )}
      ...
      {uploadState === UPLOAD_STATE.READY && (
        <div className="flex items-center justify-center w-full h-full absolute inset-0 bg-[#b0fb5d]/10 rounded-full group-hover:bg-red-500/20 transition-all">
          {previewUrl ? (
            <video src={previewUrl} className="w-full h-full object-cover rounded-full" muted playsInline />
          ) : (
            <VideoIcon className="text-[#b0fb5d]" />
          )}
          {/* Clear hover icon */}
    ...
    <div ref={containerRef} className="relative flex items-center gap-1.5">
      ...
      <button
        ...
        className={promptMediaButtonClassName({
          ...
          className: images.length > 0 ? "border-[#b0fb5d] ring-1 ring-[#b0fb5d]/40" : "",
        })}
      >
        <input
          ref={inputRef}
        ...
        {isUploading && (
          <div className="flex flex-col items-center justify-center w-full h-full absolute inset-0 bg-black/80 z-20 backdrop-blur-[2px]">
            <span className="animate-spin text-xs text-[#b0fb5d]">◌</span>
          </div>
        )}
        ...
        {images.length > 0 && (
          <div className="w-full h-full relative rounded-full overflow-hidden">
            ...
            <div className="absolute bottom-0 inset-x-0 bg-black/75 text-[9px] font-black text-[#b0fb5d] text-center leading-3">
              {images.length}
            </div>
          </div>
        )}
      ...
      {images.length > 0 && images.length < maxImages && (
        <button
          ...
          className="w-6 h-6 rounded-full bg-white/[0.04] border border-white/10 hover:border-[#b0fb5d]/50 hover:bg-[#b0fb5d]/10 text-white/50 hover:text-[#b0fb5d] flex items-center justify-center text-xs transition-colors"
        >
          +
        </button>
      )}
        ...
        <PromptPopover className="w-72 p-3 flex flex-col gap-2.5 z-50">
          <div className="flex items-center justify-between border-b border-white/10 pb-1.5">
            ...
            <button
              type="button"
              onClick={() => inputRef.current?.click()}
              className="text-[11px] font-bold text-[#b0fb5d] hover:underline"
            >
        ...
        {["videos", "images", "results"].map((tab) => (
          <button
            ...
            className={`flex-1 py-1 text-xs font-bold rounded-lg capitalize transition-all ${
              activeTab === tab
                ? "bg-[#b0fb5d] text-black shadow-sm"
            ...
            >
              <div className="w-11 h-11 rounded-lg overflow-hidden bg-white/5 flex-shrink-0 relative border border-white/10">
                ...
                <button
                  ...
                  className="absolute inset-0 bg-black/60 opacity-0 group-hover/item:opacity-100 flex items-center justify-center transition-opacity text-white hover:text-[#b0fb5d]"
                >
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                {activeTab === "images" ? (
                  <button
                    ...
                    className="text-[11px] text-black font-black px-2.5 py-1 bg-[#b0fb5d] rounded-md hover:bg-[#b0fb5d]/90 transition-colors whitespace-nowrap shadow-sm"
                  >
        ...
        {history.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full pt-4 animate-fade-in-up">
            {history.map((entry, idx) => (
              <div
                key={entry.id || idx}
                className="relative group rounded-2xl overflow-hidden border border-white/10 bg-[#0a0a0a] shadow-xl hover:border-[#b0fb5d]/50 transition-all duration-300 flex flex-col cursor-pointer"
              ...
              >
                ...
                <div className="absolute top-2 right-2 hidden md:flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  ...
                  <button
                    ...
                    className="p-2 bg-black/60 backdrop-blur-md rounded-full text-white hover:bg-[#b0fb5d] hover:text-black transition-all border border-white/10"
                  >
                  ...
                  <button
                    ...
                    className="p-2 bg-black/60 backdrop-blur-md rounded-full text-[#b0fb5d] hover:bg-[#b0fb5d] hover:text-black transition-all border border-white/10"
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                ...
                <div className="p-3 bg-black/80 backdrop-blur-sm border-t border-white/5 flex-1 flex flex-col justify-between gap-2">
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-1 mt-1">
                    <span className="text-[10px] font-bold text-[#b0fb5d] px-2 py-0.5 bg-[#b0fb5d]/10 rounded border border-[#b0fb5d]/20 whitespace-nowrap">
          ...
          <div className="flex flex-col items-center justify-center h-full animate-fade-in-up transition-all duration-700 min-h-[50vh] relative">
            ...
            <div className="absolute w-96 h-96 bg-[#b0fb5d]/5 rounded-full blur-3xl pointer-events-none -z-10" />
            ...
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-black tracking-tight mb-3 text-center px-4 flex flex-col items-center">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-[#b0fb5d] to-white font-black uppercase tracking-tight">
        ...
        <PromptFooter>
          <PromptControls>
            ...
            <div className="relative">
              ...
              >
                <div className="w-3.5 h-3.5 bg-[#b0fb5d] rounded-sm flex items-center justify-center flex-shrink-0">
              ...
              {openDropdown === "duration" && (
                ...
                >
                  ...
                  <input
                    ...
                    className="w-full accent-[#b0fb5d] cursor-pointer"
                  />
                    ...
                    {[5, 10, 15, 20, 30].filter((d) => d <= maxDurationAllowed).map((d) => (
                      <button
                        ...
                        className={`px-2.5 py-1 text-xs font-semibold rounded-lg transition-all ${
                          duration === d
                            ? "bg-[#b0fb5d] text-black font-bold shadow-sm"
            ...
            >
              <AudioIcon enabled={generateAudio} className={generateAudio ? "text-[#b0fb5d]" : "text-white/40"} />
              <span className={PROMPT_CONTROL_LABEL_CLASS}>
                ...
                >
                    ...
                    <div className="flex items-center justify-between gap-2 p-2 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                      ...
                      <button
                        ...
                        className={`w-10 h-5 rounded-full transition-colors relative p-0.5 ${
                          highBitrate ? "bg-[#b0fb5d]" : "bg-white/15"
                        }`}
                      >
                        <div
                  ...
                  <div className="flex items-center justify-between gap-2 p-2 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                    ...
                    <input
                      ...
                      placeholder="-1"
                      className="w-20 px-2 py-1 text-xs bg-black/60 border border-white/10 rounded-lg text-white text-right focus:outline-none focus:border-[#b0fb5d]/50"
                    />
                  </div>
                </PromptPopover>
              )}
            ...
            <div className="relative">
              ...
              >
                <svg
                  ...
                  stroke="currentColor"
                  strokeWidth="2"
                  className="text-white/50 group-hover:text-[#b0fb5d] transition-colors"
                >
