...
const SELECT_CLASS = "w-full rounded-lg border border-white/10 bg-[#17191c] px-3 py-2 text-xs text-white outline-none focus:border-[#b0fb5d]/50";
                ...
                description={(option.description || option.adjustmentDescription || option.disabled) && (
                  <span className="block">
                    ...
                    {option.adjustmentDescription && (
                      <span className="block text-[#b0fb5d]/75">{option.adjustmentDescription}</span>
                    )}
...
export function VideoSettingsControl({
  ...
  const renderProfile = (option) => {
    ...
    return (
      <label
        key={option.value}
        className={`flex items-start gap-3 rounded-lg px-3 py-2 text-xs transition-colors focus-within:ring-1 focus-within:ring-[#b0fb5d]/50 ${
          option.disabled ? "cursor-not-allowed opacity-40" : "cursor-pointer hover:bg-white/[0.04]"
        } ${profile.value === option.value ? "bg-[#b0fb5d]/[0.07]" : ""}`}
      >
        <input
          type="radio"
          name={profileGroupId}
          ...
          className="mt-0.5 h-3.5 w-3.5 shrink-0 accent-[#b0fb5d]"
        />
    ...
    <fieldset className="min-w-0">
      ...
      {onDefaultResolution && (
        <button type="button" onClick={onDefaultResolution}
          className="mt-2 px-3 py-2 text-xs text-[#b0fb5d] hover:underline focus-visible:outline focus-visible:outline-1 focus-visible:outline-[#b0fb5d]">
