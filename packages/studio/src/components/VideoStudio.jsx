...
function ReferenceMediaLabel({ label, required = false }) {
  ...
  return (
    ...
    >
      {label}
      {required && (
        <span className="ml-0.5 text-[#b0fb5d]" aria-hidden="true">
          *
        </span>
      )}
    </span>
      ...
      >
        {uploading ? (
          <div className="flex flex-col items-center justify-center w-full h-full absolute inset-0 bg-black/80 z-20 backdrop-blur-[2px]">
            ...
            <span className="absolute text-[9px] font-black text-[#b0fb5d] leading-none">{progress}%</span>
          </div>
        ) : type === "video" ? (
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="text-white/40 group-hover:text-[#b0fb5d] transition-colors">
...
const VideoReadySvg = () => (
  ...
  >
    <polygon points="23 7 16 12 23 17 23 7" />
    <rect x="1" y="5" width="15" height="14" rx="2" ry="2" />
    <polyline points="7 10 10 13 15 8" stroke="#b0fb5d" strokeWidth="2.5" />
  </svg>
);
...
function ModelDropdown({ selectedModel, onSelect, onClose, copy = en }) {
  ...
  const getProviderStyle = (provider) => {
    switch (provider) {
      ...
      case "google":
        return { text: "G", bg: "bg-lime-500/10 text-lime-400 border-lime-500/25" };
      case "blackforest":
      ...
      case "midjourney":
        return { text: "MJ", bg: "bg-lime-500/10 text-lime-400 border-lime-500/25" };
      case "kling":
        return { text: "KL", bg: "bg-rose-500/10 text-rose-400 border-rose-500/25" };
      case "vidu":
        return { text: "VD", bg: "bg-lime-500/10 text-lime-400 border-lime-500/25" };
      case "minimax":
      ...
      case "alibaba":
        return { text: "AL", bg: "bg-lime-500/10 text-lime-400 border-lime-500/25" };
      case "leonardoai":
  ...
  const getIconColor = (family) => {
    if (family.id.includes("kling")) return "bg-lime-500/10 text-lime-400 border-lime-500/10";
              ...
              {canUploadImageReference && (
                ...
                >
                  <svg
                    ...
                    stroke="currentColor"
                    strokeWidth="2.5"
                    className="opacity-40 text-white group-hover:text-[#b0fb5d] transition-colors"
                  >
                    <path d="M12 20h9" />
