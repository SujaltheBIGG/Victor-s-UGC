...
// A missing or rejected key is an expected state, not a fault: the app renders
// its signed-out UI and retries once a usable key arrives. Tagging the error
// lets callers report real failures loudly and auth failures quietly.
function requestError(label, status, detail) {
    const err = new Error(`${label}: ${status} - ${String(detail).slice(0, 100)}`);
    err.status = status;
    err.authRequired = status === 401 || status === 403;
    return err;
}
...
export async function getUserBalance(apiKey) {
    ...
    if (!response.ok) {
        const errText = await response.text();
        notifyAuthRequired(response.status, errText);
        throw requestError('Failed to fetch balance', response.status, errText);
    }
...
export async function getAppInterests(apiKey) {
    ...
    if (!response.ok) {
        const errText = await response.text();
        notifyAuthRequired(response.status, errText);
        throw requestError('Failed to fetch interests', response.status, errText);
    }
    return await response.json();
}
